...
- **Gotcha — vitest double-counts when a worktree exists.** With a git worktree under `.claude/worktrees/`, a bare `vitest run` from the repo root scans the nested copy too, counting every suite twice and adding 2 phantom "failed files" where it tries to load the Playwright specs. Always use `npx vitest run --exclude "**/.claude/**"` while a worktree is checked out, or trust the CI run.
